...
### Checking the gateway's verdict on the machine it forwarded to
...
- Fetched by id, a session carries the evidence inline as a `data:` URI: the upstream's complete ACI report, quote included. `verifyAttestedSession()` checks the digest, then verifies that report through `verifyRelayedAciAttestation()` — DCAP against Intel's roots, digest and workload-id recomputation, the endorsement, the debug bit — and folds the results in under an `upstream.` prefix. So the second hop is verified by the relying party rather than asserted by the first.
- `channel_binding_in_attested_keyset` is the check that makes the rest mean something: the TLS SPKI the gateway bound the channel to must appear in the upstream's *attested* keyset for the host that was dialled. Without it a genuine report for some other machine would pass.
- Added `verifyRelayedAciAttestation()` and `AciAttestationResult.nonceBound` for reports obtained second-hand. The nonce the gateway sent is not published, so the statement binding cannot be recomputed and a captured report cannot be told from a current one. That check is skipped rather than faked, `nonceBound` is false, and no anchor is returned: freshness of the second hop rests on the attested gateway having behaved, bounded by the session's own expiry.
