...
export function createNrasTokenVerifier(options = {}) {
    const { jwksUrl = NRAS_JWKS_URL, cacheTtlMs = DEFAULT_CACHE_TTL_MS, clockSkewSec = DEFAULT_CLOCK_SKEW_SEC, pinnedLeafCertSha256 = [], fetchImpl, now = () => Date.now(), } = options;
    const pinnedLeaves = new Set(pinnedLeafCertSha256.map((digest) => {
