...
function validateContext(context) {
    ...
    if (!request && !response)
        throw new Error('Unsupported ACI E2EE field purpose');
...
export function aciE2eeAad(context) {
    validateContext(context);
    return encoder.encode(jcsStringify({
...
export function createAciE2eeClientKeyPair(options = {}) {
...
export function generateAciE2eeNonce() {
    return toHex(secureRandomBytes(32));
}
/** Encrypt one UTF-8 field for the quote-bound ACI E2EE public key. */
export async function encryptAciE2eeField(plaintext, recipientPublicKeyHex, context, options = {}) {
    if (typeof plaintext !== 'string')
        throw new TypeError('ACI E2EE plaintext must be a string');
    validateContext(context);
...
export async function decryptAciE2eeField(ciphertextHex, recipientSecretKey, context) {
    validateContext(context);
...
//# sourceMappingURL=aci-e2ee.js.map